...
def draw_identified_LASregions(pair):
	chr1, chr2 = pair
...
def run_LAS(pair, threshold_new):
	chr1, chr2 = pair
	fname = config["HIC_NEW_DIR"] + 'intermingling_regions.chr' + str(chr1) + '_chr' + str(chr2) +'.avg_' + "{0:.4g}".format(threshold_new) + '.csv'
	if (os.path.isfile(fname) == False):
		...
		df_intermingling.to_csv(config["HIC_NEW_DIR"]  + 'intermingling_regions.chr' + str(chr1) + '_chr' + str(chr2) + '.csv')		
...
def main():
	...
	# draw submatrices
	Parallel(n_jobs = config['NUM_PROC'])(delayed(draw_identified_LASregions)(pair) for pair in chr_pairs)  
	# calculate score
